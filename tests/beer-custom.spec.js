...
test("bar chart widget renders an SVG in the card body", async ({ page }) => {
  await openCreateModal(page);

  // Select bar chart type
  await page.locator("#beer-custom-type-picker button", { hasText: "Bar chart" }).click();
  await page.fill("#beer-custom-title", "Check-ins Over Time");
  await page.click("#beer-custom-save");

  await expect(page.locator("#beer-custom-builder")).not.toBeVisible();
  // roughViz renders bars as SVG paths inside the card body
  await expect(page.locator("#beer-custom-grid .card-body svg")).toBeVisible({ timeout: 8000 });
});

test("heatmap widget renders day cells in the card body", async ({ page }) => {
  await openCreateModal(page);

  await page.locator("#beer-custom-type-picker button", { hasText: "Heatmap" }).click();
  await page.fill("#beer-custom-title", "Activity Heatmap");
  await page.click("#beer-custom-save");

  await expect(page.locator("#beer-custom-builder")).not.toBeVisible();
  // InsightsHeatmap.init creates .heatmap-day cells inside the card body
  await expect(page.locator("#beer-custom-grid .card-body .heatmap-day").first()).toBeVisible({
    timeout: 3000,
  });
});

test("stat card widget renders total check-in count as a number", async ({ page }) => {
  await openCreateModal(page);

  await page.locator("#beer-custom-type-picker button", { hasText: "Stat card" }).click();
  await page.fill("#beer-custom-title", "Total Check-ins");
  await page.click("#beer-custom-save");

  await expect(page.locator("#beer-custom-builder")).not.toBeVisible();
  // Stat card renders a display-5 number (9 check-ins in the fixture)
  const statEl = page.locator("#beer-custom-grid .card-body .display-5");
  await expect(statEl).toBeVisible();
  await expect(statEl).toContainText("9");
});

test("donut widget with brewery groupBy renders an SVG in the card body", async ({ page }) => {
  await openCreateModal(page);

  await page.locator("#beer-custom-type-picker button", { hasText: "Donut" }).click();
  // Donut requires a groupBy to produce groups; select brewery
  await page.selectOption("#beer-cfg-groupBy", "brewery");
  await page.fill("#beer-custom-title", "By Brewery");
  await page.click("#beer-custom-save");

  await expect(page.locator("#beer-custom-builder")).not.toBeVisible();
  // roughViz renders a donut chart as SVG inside the card body (two SVGs are created; use first())
  await expect(page.locator("#beer-custom-grid .card-body svg").first()).toBeVisible({
    timeout: 8000,
  });
});
