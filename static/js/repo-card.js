/**
 * From: https://tarptaeya.github.io/repo-card/repo-card.js
 * 
 * Modified by Rees Draminski
 * 
 * 06/19/2020: Allow a text-decoration: underline to appear on hover for repo name
 */
window.addEventListener('DOMContentLoaded', async () => {
    let resp;
    
    resp = await fetch('https://api.github.com/emojis');
    let emojis = await resp.json();
    
    resp = await fetch('https://raw.githubusercontent.com/ozh/github-colors/master/colors.json');
    let colors = await resp.json();
    
    document.querySelectorAll('.repo-card').forEach(async (el) => {
        let name = el.getAttribute('data-repo');
        ...
        <div style="display: flex; align-items: center;">
            ...
            <a style="color: inherit;" href="${data.html_url}">${data.name}</a>
            </span>
        </div>
        <div style="display: ${data.fork ? 'block' : 'none'}; font-size: 12px; color: #586069;">Forked from <a style="color: inherit;" href="${data.fork ? data.source.html_url : ''}">${data.fork ? data.source.full_name : ''}</a></div>
