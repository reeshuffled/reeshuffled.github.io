...
async function loadData() {
  ...
  remainingLetters.split("").forEach((letter) => {
...
function guessWord(wordData, guess) {
  ...
  if (currentCellIndex < guessBoardCells.length) {
    ...
    const nextCell =
      currentCell + 1 != guessBoardCells.length - 1 ? guessBoardCells[currentCellIndex + 1] : null;
    currentCellIndex++;
...
function showFriendGuesses(wordData) {
  ...
  for (let i = 0; i < friendGuesses.length; i++) {
    ...
    if (hit) {
      cells[i].innerHTML +=
        `<p><span style="color: blue">${guess}</span><span style="float: right">${hit.info}</span></p>`;
    } else {
      cells[i].innerHTML +=
        `<p><span style="color: blue">${guess}</span><span style="float: right">X</span></p>`;
    }
  }
}
...
function decodeFriendGuesses(encodedGuesses) {
  return encodedGuesses.split(",").map((guess) => {
    return guess
      .split("")
      .map((letter) => {
        // if is letter, shift by amount
